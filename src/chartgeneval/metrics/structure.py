...
* ``boredom_v2`` -- a stagnation--alienation structural proxy, replacing the
  single-sided incumbent ``boredom_rate`` heuristic as the primary structural
  read (the incumbent stays as a raw diagnostic). The code key is archival;
  the output has no player-boredom criterion-validity claim.
...
def call_response_reciprocity(events, ctx):
    ...
    Operationalization: ABAB-style phrase return, where material from one
    two-bar phrase returns after an intervening phrase. Phrases are quantised to
    16th-note slots.
...
def boredom_v2(events, ctx):
    """Stagnation--alienation structural proxy (archival key: boredom_v2).

    Bar-level rhythm/colour fingerprints come from the metrical grid and are
    deliberately decoupled from the LM event-token stream the pattern family
    shares. The proxy marks a bar when it is either:
