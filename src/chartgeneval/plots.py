...
def plot_profile(
    ...
    *,
    ...
    pass/fail glyph block right of the heatmap; a failed constraint outlines
    the system row in red (gates veto, they do not average in).
    ...
    for k, name in enumerate(con_names):
        ...
        if constraints and any((constraints.get(s) or {}).get(name) is False for s in systems):
            pass
    for i, s in enumerate(systems):
        if constraints and any(v is False for v in (constraints.get(s) or {}).values()):
            ax.add_patch(
                plt.Rectangle(
