...
from the training split of ``taiko-1000-parsed-clean``; these are summary
quantiles, not raw charts). :func:`load_bundled_calibration` returns it. Users
can rebuild bands from their own corpus with :func:`build_calibration`.
"""
...
BUNDLED_CALIBRATION = "taiko_1000_parsed_clean_calibration_v2.json"
...
def load_bundled_calibration() -> dict:
    ...
    with resources.files("chartgeneval.data").joinpath(BUNDLED_CALIBRATION).open() as f:
        return _validate_calibration(json.load(f), BUNDLED_CALIBRATION)


def load_calibration(path: str | Path) -> dict:
    """Load a calibration artifact from an arbitrary path."""
    with open(path) as f:
        return _validate_calibration(json.load(f), str(path))


def _validate_calibration(calibration: dict, source: str) -> dict:
    """Reject calibration artifacts built for a different scoring contract."""
    version = calibration.get("score_version")
    if version != SCORE_VERSION:
        raise ValueError(
            f"incompatible calibration {source!r}: score_version={version!r}; "
            f"expected {SCORE_VERSION!r}"
        )
    courses = calibration.get("courses")
    if not isinstance(courses, dict) or not courses:
        raise ValueError(f"invalid calibration {source!r}: missing non-empty 'courses' mapping")
    required = {
        "artifact_version": 1,
        "split": "train",
    }
    for key, expected in required.items():
        if calibration.get(key) != expected:
            raise ValueError(
                f"invalid calibration {source!r}: {key}={calibration.get(key)!r}; "
                f"expected {expected!r}"
            )
    if not isinstance(calibration.get("dataset_id"), str) or not calibration["dataset_id"]:
        raise ValueError(f"invalid calibration {source!r}: missing dataset_id")
    if not re.fullmatch(r"[0-9a-f]{40}", calibration.get("dataset_revision", "")):
        raise ValueError(f"invalid calibration {source!r}: dataset_revision is not a commit hash")
    if not isinstance(calibration.get("n_charts"), int) or calibration["n_charts"] <= 0:
        raise ValueError(f"invalid calibration {source!r}: n_charts must be positive")
    if not isinstance(calibration.get("lm_order"), int) or calibration["lm_order"] < 2:
        raise ValueError(f"invalid calibration {source!r}: lm_order must be an integer >= 2")
    lm_alpha = calibration.get("lm_alpha")
    if not isinstance(lm_alpha, (int, float)) or not math.isfinite(lm_alpha) or lm_alpha <= 0:
        raise ValueError(f"invalid calibration {source!r}: lm_alpha must be finite and positive")
    return calibration


def calibration_content_sha256(calibration: dict) -> str:
    """Hash a calibration by canonical JSON content, independent of formatting."""
    payload = json.dumps(
        calibration,
        ensure_ascii=False,
        sort_keys=True,
...
def require_calibration_provenance(
    ...
    *,
    ...
    """Fail if an LM runner and calibration do not share one pinned corpus."""
    expected = {
...
def _course_calibration(calibration, course):
    ...
    if course not in courses:
        available = ", ".join(sorted(courses)) or "none"
        raise ValueError(
            f"no calibration for course {course!r}; available courses: {available}"
        )
...
def score_chart_features(features, calibration, course) -> dict:
    ...
    def band(metric, score_name):
        ...
        if lower is None or upper is None:
            ...
            return
        scores[score_name] = band_score(features.get(metric), lower, upper)
