...
0.2ms 以內。我們釋出探針、校準 artifact、全部逐譜紀錄、可安裝的
`chartgeneval` 套件，以及底層資料集的洩漏稽核重切分版。
...
劑量增長。這把指標驗證變成便宜、可重現的單元測試——任何新指標一個下午
就能跑完。
...
4. **釋出 artifact**：探針實作、校準帶、全部 4,175 筆逐譜紀錄、
   `pip install chartgeneval` 套件（本 repo）、以及資料集的洩漏稽核重切分
   （我們發現並記錄了原切分 21.6% 的歌曲層 train/test 重疊）

**刻意不在範圍內**：人類效度。探針確立的是指標「偵測損壞」的能力；存活
指標是否追蹤「玩家體驗」是感知研究的問題，我們預註冊為後續工作——劣化
算子的設計刻意對齊了計畫中的人類研究操縱，屆時評分可直接接入成為第六個
驗收準則。**本文亦不引入我們自己的生成模型**：應用節（§5.5）評估的是
既有公開系統，消除自評偏誤。
...
**資料。** 1,155 張 taiko 風格譜面。出處稽核（三鍵：標題、音訊路徑、音訊
SHA-256）發現 116 首測試歌中 25 首也出現在訓練切分；我們隨論文釋出群組化、
分層的乾淨重切分（813/80/120 首唯一歌曲，`taiko-1000-parsed-clean`）。
...
**尚無人類效度。** 探針認證損壞偵測，不認證體驗預測。操縱對齊本文探針
的感知研究已預註冊為後續；其評分將成為額外驗收準則，屆時任何在此考核
表現良好但人類效度不過的分數一律降級。
...
（未到 level）的帶、以及 LM 訓練語料與受評歌曲有實測 17.4% 的重疊——
經半語料擾動證明不承重（系統排序 Spearman 1.000、AUC 位移 ≤0.078），
嚴格 leak-free LM 為未跑的更強版本。
...
譜面生成需要的是可信的評估，多於需要另一個指標。劣化探針讓指標驗證
變得便宜而鮮明：借來的指標不只是表現不佳——它們獎勵平庸、獎勵崩潰、
並完全錯過致命的整體錯位。同樣的 raw 量經語料校準帶正規化、加上外部
格線錨定的時值家族，以單調劑量反應與次毫秒平移回收通過同一套探針，
其耦合結構公開而非隱藏。harness、校準 artifact、逐譜紀錄、`chartgeneval`
套件與洩漏稽核後的乾淨資料集重切分全部釋出——下一個譜面指標可以先被
考核，再被相信。
