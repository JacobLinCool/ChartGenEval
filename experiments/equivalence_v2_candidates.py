...
DEFAULT_SOFTCHART = Path("/Users/jacoblincool/Documents/GitHub/SoftChart")
...
def main():
    ...
    ap.add_argument("--softchart", type=Path, default=DEFAULT_SOFTCHART)
    ...
    # fit ONE shared manifold reference from the ported phi over all cases
    # (both sides score against the byte-identical ref dict)
    phis = [gap.phi(ev, ctx.get("bpm")) for _, ev, ctx in cases]
    ref = gap.fit_manifold_ref([p for p in phis if p is not None], k=5)

    report = {}
    for name, ev, ctx in cases:
        ctx_ref = dict(ctx)
        ctx_ref["official_manifold_ref"] = ref
