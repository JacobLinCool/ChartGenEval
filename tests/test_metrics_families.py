...
def test_band_score_edges():
    ...
    # one half-width beyond either edge has exactly the same score
    assert band_score(3.0, 4.0, 6.0) == pytest.approx(0.5)
    assert band_score(7.0, 4.0, 6.0) == pytest.approx(0.5)
...
def test_timing_empty_grid_graceful(alt_eighths, bpm):
    ...
    # BPM identifies period but not phase; deriving phase from the evaluated
    # chart would make every global shift look perfect.
    assert out["n_matched"] == 0
    assert out["grid_phase_offset_ms"] is None
    assert out["grid_phase_offset_abs_ms"] is None


def test_timing_clean_rate_does_not_claim_deletion_resistance(alt_eighths, bpm, grid):
    """The timing rate is precision-like; it has no reference-note recall term."""
    ctx = make_ctx(course="oni", bpm=bpm, grid=grid, duration=64.0)
    intact = timing.compute(alt_eighths, ctx)
    with_unmatched_note = timing.compute([*alt_eighths, (100.0, "don")], ctx)
    assert with_unmatched_note["n_unsupported"] == intact["n_unsupported"] + 1
    assert with_unmatched_note["clean_rate"] < intact["clean_rate"]
    # Removing that unsupported generated note restores the higher rate. The
    # paper therefore treats deletion resistance as unmeasured, not guaranteed.
...
def test_repetition_and_surface_variety_are_one_effective_axis(constant_dons, bpm):
    """Document the C5 post-freeze dependency; these are not two metrics."""
    calibration = load_bundled_calibration()
    for course in ("easy", "normal", "hard", "oni", "ura"):
        bands = calibration["courses"][course]["bands"]
        repeated = bands["repeat_4gram_rate"]
        unique = bands["unique_4gram_rate"]
        assert repeated["p10"] == pytest.approx(1.0 - unique["p90"], abs=1e-15)
        assert repeated["p90"] == pytest.approx(1.0 - unique["p10"], abs=1e-15)

    out = structure.compute(
        constant_dons,
        make_ctx(course="oni", bpm=bpm, calibration=calibration),
    )
    assert out["repeat_4gram_rate"] + out["unique_4gram_rate"] == pytest.approx(1.0)
    assert out["repetition_adequacy_score"] == pytest.approx(
        out["surface_variety_adequacy_score"], abs=1e-15
    )
    assert out["surface_structure_proxy_score"] == pytest.approx(
        geometric_mean(
            [
                out["repetition_adequacy_score"],
                out["surface_variety_adequacy_score"],
                out["density_variation_adequacy_score"],
            ]
        )
    )
...
def test_gap_official_scores_higher_than_outlier(alt_eighths, constant_dons, bpm):
    ...
    ref = gap.fit_manifold_ref(
        [gap.phi(c, bpm) for c in ref_charts], course="oni", k=3
    )
...
def test_gap_rejects_wrong_course_reference(alt_eighths, bpm):
    vectors = [
        gap.phi([(i * 60.0 / bpm, "don" if (i + s) % 2 else "ka") for i in range(64)], bpm)
        for s in range(3)
    ]
    ref = gap.fit_manifold_ref(vectors, course="hard", k=2)
    with pytest.raises(ValueError, match="does not match"):
        gap.compute(
            alt_eighths,
            make_ctx(course="oni", bpm=bpm, official_manifold_ref=ref),
        )


# ------------------------------------------------------ calibrated profile ----
def test_evaluate_profile_has_leaves_and_all_families(alt_eighths, bpm, toy_lm, toy_calibration):
    from chartgeneval.metrics import evaluate_profile

    ctx = make_ctx(course="oni", bpm=bpm, lm=toy_lm, calibration=toy_calibration)
    profile = evaluate_profile(alt_eighths, ctx)
    assert 0.0 <= profile["density_adequacy_score"] <= 1.0
    assert "chart_quality_proxy_score" not in profile
    for family in ("timing", "coupling", "grammar", "structure", "gap", "constraints"):
        assert any(key.startswith(f"{family}.") for key in profile)


def test_evaluate_profile_rejects_unknown_course(alt_eighths, bpm, toy_lm, toy_calibration):
    from chartgeneval.metrics import evaluate_profile

    ctx = make_ctx(course="unknown", bpm=bpm, lm=toy_lm, calibration=toy_calibration)
    with pytest.raises(ValueError, match="no calibration for course"):
        evaluate_profile(alt_eighths, ctx)
